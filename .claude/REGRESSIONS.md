...
| New Arrivals answers a trackpad by STEPPING one garment, not by pixel-dragging like the other rails — its panel widths rearrange as the window moves, so a raw drag snaps somewhere Owl disagrees with | `pages/home.php` | Two fingers sideways moves one garment; a flick still moves one |
