...
| New Arrivals is an Owl carousel with `slideBy: 1` — one garment per press, four across above 1025px and one with a peek below. Captions show AT REST, not on hover: with equal panels there is no permanently-open one, and a touchscreen has no hover, so hover-only captions mean a phone never shows a name or price | `pages/home.php`, `assets/css/style.css` (the appended `.na-gallery.owl-loaded` block, which must stay LAST in the file) | Counter reads `1 / 5` on desktop; every visible tile shows its name and price without hovering |
