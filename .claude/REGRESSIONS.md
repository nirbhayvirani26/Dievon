...
| On mobile every rail matches: 12px gutter, ~two thirds of the screen per slide, 40px section padding | `assets/css/style.css`, `pages/home.php` | Measured equal across New Arrivals, Occasions and the fan |
...
| Quick View is centred with `left:0; right:0; margin-inline:auto` — NEVER `left:50%`, which shrink-to-fits an absolute box to the space from the centre to the right edge and spills nowrap text out of its own button. The fan broke this by setting `left:50%` without clearing the inherited `right:0` | `assets/css/style.css` | Button sits centred on every print |
