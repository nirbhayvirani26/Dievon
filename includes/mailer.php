...
function sendPaymentReceiptEmail(array $order): bool {
    try {
        // Routed to a real receipt template. This used to pass 'Delivered' into
        // sendOrderStatusEmail(), which claimed the order had been delivered and
        // then blocked the genuine delivery email later. See the note on
        // EmailService::sendPaymentReceivedEmail().
        return getEmailService()->sendPaymentReceivedEmail($order);
    } catch (\Throwable $e) {
