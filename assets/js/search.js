...
document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('ajax_search.php?q=' + encodeURIComponent(query))
                ...
                .then(data => {
                    ...
                    data.forEach(product => {
                        ...
                        html += `
                            <a href="product.php?id=${product.id}" class="search-product-card">
                                ${imgHtml}
    ...
    function loadRecentSearches() {
        ...
        let html = '';
        searches.forEach(s => {
            html += `<li><a href="shop?search=${encodeURIComponent(s)}"><i class="fa-solid fa-clock-rotate-left" style="font-size:12px; margin-right:8px; color:var(--text-muted);"></i> ${escapeHtml(s)}</a></li>`;
        });
