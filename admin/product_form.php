...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isPublishing) {
        ...
        foreach (['silk','cotton','linen','georgette','crepe','organza','satin','brocade','chiffon','velvet','rayon','chanderi','muslin'] as $fw) {
            if (str_contains($nameLower, $fw) && $fabricLower !== '' && $fabricLower !== $fw && !$confirmIncomplete) {
                // Publish Anyway is the owner's explicit call to put it live
        ...
        if (trim($wash_care) === '')     { $incomplete[] = 'care instructions'; }
        if (trim($dimensions) === '')    { $incomplete[] = 'dimensions'; }
        if (trim((string)$weight) === '' || (float)$weight <= 0) { $incomplete[] = 'shipping weight'; }
        ...
        // Tax status. Without HSN and a rate the invoice cannot be a tax invoice
        // and prints as an "Order Summary" — which is legal, but not what a
        // GST-registered seller wants going out.
        if ((trim((string)$hsn_code) === '' || (float)$gst_rate <= 0) && !$confirmIncomplete) {
            $publishWarnings[] = 'No HSN code or GST rate. Set the collection\'s defaults under Categories, or uncheck '
                               . '"Use Category Tax Settings" and set this product\'s own (or press Publish Anyway) — without '
                               . 'them the invoice prints as an "Order Summary", not a proper tax invoice — legal, but not '
                               . 'what a GST-registered seller wants going out.';
        }
                    ...
                    <div class="glass-panel form-section admin-section-mt<?= $hasColourways ? ' section-superseded' : '' ?>">
                        ...
                        <div class="admin-gallery-wrap-flex" id="additionalImagesContainer">
                            ...
                            <div class="gallery-image-wrap admin-gallery-thumb-item" id="ai-<?= $img['id'] ?>">
                                <img src="<?= SITE_URL ?>/uploads/products/<?= htmlspecialchars($img['image']) ?>" class="admin-gallery-thumb-img">
...
function updateColorSizePrice(colorId, code) { saveColorSize(colorId, code); }
